...
let tag (p : 'a program) : tag program =
  ...
  let rec helpE (e : 'a expr) : tag expr =
    ...
    | EInline(args, body, _) ->
       let lam_tag = tag() in
       EInline(List.map (fun (a, _) -> (a, tag())) args, helpE body, lam_tag)
    | ELambda(args, body, _) ->
       let lam_tag = tag() in
...
let rec untag (p : 'a program) : unit program =
  let rec helpE (e : 'a program) =
    ...
    | EInline(args, body, _) ->
       EInline(List.map (fun (x, _) -> (x, ())) args, helpE body, ())
    | ELambda(args, body, _) ->
...
let atag (p : 'a aprogram) : tag aprogram =
  ...
  and helpC (c : 'a cexpr) : tag cexpr =
    match c with
    | CPrim1(op, e, _) -> let prim_tag = tag() in CPrim1(op, helpI e, prim_tag)
    | CPrim2(op, e1, e2, _) -> let prim_tag = tag() in CPrim2(op, helpI e1, helpI e2, prim_tag)
    | CIf(cond, thn, els, _) -> let if_tag = tag() in CIf(helpI cond, helpA thn, helpA els, if_tag)
    | CAExpr(br) -> CAExpr(helpA br)
    | CTuple(vals, _) -> let tuple_tag = tag() in CTuple(List.map helpI vals, tuple_tag)
    | CGetItem(tup, idx, _) -> let get_tag = tag() in CGetItem(helpI tup, helpI idx, get_tag)
    | CSetItem(tup, idx, rhs, _) -> let get_tag = tag() in CSetItem(helpI tup, helpI idx, helpI rhs, get_tag)
    | CApp(name, args, _) -> let app_tag = tag() in CApp(helpI name, List.map helpI args, app_tag)
    | CInline(args, body, _) -> let lam_tag = tag() in CInline(args, helpA body, lam_tag)
    | CLambda(args, body, _) -> let lam_tag = tag() in CLambda(args, helpA body, lam_tag)
