...
let rec string_of_expr (e : 'a expr) : string =
  ...
  | EInline(args, body, _) ->
     sprintf "(inline(%s): %s)" (ExtString.String.join ", " (List.map fst args))
             (string_of_expr body)
  | ELambda(args, body, _) ->
...
let rec string_of_aexpr (e : 'a aexpr) : string =
  match e with
  | ASeq(fst, snd, _) ->
     sprintf "(Seq: %s; %s)" (string_of_cexpr fst) (string_of_aexpr snd)
...
and string_of_cexpr c =
  ...
  | CIf(cond, thn, els, _) ->
     sprintf "(if %s: %s else: %s)" (string_of_immexpr cond) (string_of_aexpr thn) (string_of_aexpr els)
  | CAExpr br ->
     sprintf "(aexpr: %s)" (string_of_aexpr br)
  | CApp(func, args, _) ->
  ...
  | CInline(args, body, _) ->
     sprintf "(inline(%s): %s)" (ExtString.String.join ", " args) (string_of_aexpr body)
